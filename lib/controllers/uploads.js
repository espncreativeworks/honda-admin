...
exports.list = function (req, res, next) {
  ...
  var orientation = req.param('orientation') || 'landscape';
  ...
  query.exec(function (err, uploads) {
    ...
    uploads = uploads.filter(function(upload){
      var tests = 1,
          passed = 0,
          _video = upload.videos[0];

      if (orientation === 'landscape' && (_video.width > _video.height)){
        passed++;
      }

      if (orientation === 'portrait' && (_video.height > _video.width)){
        passed++;
      }
